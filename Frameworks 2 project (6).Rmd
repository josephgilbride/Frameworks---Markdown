...
xdtm_tfidf$column = names(rawrows)
xdtm_tfidf = t(xdtm_tfidf)
xdtm_tfidf = as.data.frame(xdtm_tfidf)
xdtm_tfidf2 = rownames(xdtm_tfidf)
xdtm_tfidf= apply(xdtm_tfidf,2,function(x)as.numeric(x))
xdtm_tfidf = as.data.frame(xdtm_tfidf)
xdtm_tfidf$multiplier = rowMeans(xdtm_tfidf)
xdtm_tfidf = cbind(xdtm_tfidf, word = xdtm_tfidf2)
#Calculating weighted columns
corp_df = as.data.frame(corpus$content)
corp_df$column = names(rawrows)
corp_df$`corpus$content` = as.vector(corp_df$`corpus$content`)
corp_df2 = corp_df %>%
  unnest_tokens(word, `corpus$content`) %>%
  group_by(column) %>%
  count(word, sort = TRUE)

corp_df = inner_join(corp_df2, xdtm_tfidf)
corp_df$scores = corp_df$n*corp_df$multiplier
corp_df[,2:38] = NULL
pivot = summarise(corp_df, sum_amount = sum(scores))
row.names(pivot) = pivot$column
pivot$column= NULL
pivot = t(pivot)
...
gov_data_scores <- rbind(mat,mat2,mat3,mat4)
gov_data_scores = as.data.frame(gov_data_scores)

#View correlations of sentiment in document
cor(gov_data_scores)
...
gov_data_stats = cbind(lengths, pivot, mat) #mat2, mat3, depending on sentiment
gov_data_stats = as.data.frame(gov_data_stats)
gov_data_stats$Award = predata$`predata`
...
set.seed(522)
split = createDataPartition(y=gov_data_stats$Award,p = 0.7,list = F,groups = 100)

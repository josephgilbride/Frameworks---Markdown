...
```{r}
rawrows = do.call(paste, as.data.frame(rawrows, stringsAsFactors=FALSE))
rawrows = as.data.frame(rawrows)
gov_data = cbind(goodrows, rawrows, Award)
```
...
corpus = Corpus(VectorSource(gov_data$rawrows))
...
cor(xdtm_tfidf, xdtm_tfidf$Award)
